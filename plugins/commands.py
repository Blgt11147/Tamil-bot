...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('⍟ PREMIUM', callback_data='Pʀᴇᴍɪᴜᴍ')
                ],[
                    InlineKeyboardButton('✇ PREMIUM ✇', callback_data='PREMIUM')
                  ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        buttons = [[
                    ...
                    InlineKeyboardButton('〄 Hᴇʟᴘ', callback_data='help'),
                    InlineKeyboardButton('⍟ Pʀᴇᴍɪᴜᴍ', callback_data='Pʀᴇᴍɪᴜᴍ')
                ],[
                    InlineKeyboardButton('✇ PREMIUM ✇', callback_data='PREMIUM')
                  ]]
        reply_markup = InlineKeyboardMarkup(buttons)      
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          ...
                          InlineKeyboardButton("Mᴏᴠɪᴇ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ", url="https://t.me/baljeetsingh_11147")
                         ]
                        ]
                    )
                )
                    ...
                    InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton("Mᴏᴠɪᴇ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ", url="https://t.me/baljeet_singh1114")
                            ]
                        ]
                    )
                    if IS_STREAM
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@baljeet_singh1114 {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
